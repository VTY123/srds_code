...
def get_watersdata(filename):
	f = open(DIR+'/mst_ibe_data/'+filename,'r')
	s = f.read()
	arr = s.split('\n')
	arr = arr[0:-1]
	xa = []
...
def bar_acc():
	...
	zkclientdata = get_watersdata('pkVerify_client.txt')
	zkserverdata = get_watersdata('pkVerify_server.txt')	
	kCombinedata = get_watersdata('kCombine.txt')
	#kCombineclientdata = get_watersdata('kCombine_client.txt')
	testkeydata = get_watersdata('dkVerify.txt')
		#r = acc(keyderdata[1][i],keysetupdata[1][i])
		#r = acc(r,pCombinedata[1][i])
		#r = acc(r,kCombineclientdata[1][i+kCombineserverdata[1])
...
def bar_plot():
	...
	pCombinedata = get_watersdata('pCombine.txt')
	#zkclientdata = get_watersdata('pkVerify_client.txt')
	pkVerifydata = get_watersdata('pkVerify.txt')
	plt.bar(br,keysetupdata[1][i],width=barWidth,label='KeySetup', edgecolor = plt.cm.Greys(1.0),color= plt.cm.Greys(1.0),linestyle='-')
	plt.bar(br1,keyderdata[1][i],width=barWidth,label='KeyDer',fill = False ,color= plt.cm.Greys(1.0))
	plt.bar(br2,pCombinedata[1][i],width=barWidth,label='pCombine',color= plt.cm.Greys(.6))
	...
	plt.bar(br3,kCombinedata[1][i],width=barWidth,label='kCombine',color= plt.cm.Greys(.3))	

	zkdata = pkVerifydata[1][i] #zkclientdata[1][i]+
	plt.bar(br4,zkdata,width=barWidth,label='pkVerify',color= plt.cm.Greys(.0),hatch='//')

	testkeydata = get_watersdata('dkVerify.txt')
	plt.bar(br5,testkeydata[1][i],width=barWidth,label='dkVerify',color= plt.cm.Greys(.3),hatch='//')
...
#		print(k)
	...
	#ystr = [str(int(keysetupdata[0][x]))+' clients' for x in i] #+ [str(int(keysetupdata[0][x]))+' clients (Waters)' for x in i] 
